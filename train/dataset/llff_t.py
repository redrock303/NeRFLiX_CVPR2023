...
class LLFFT_Synthetic(tData.Dataset):
    def __init__(self, dataPath = '/newdata/kunzhou/dataset/vfi_benchmarks/nerf_llff_data',nframes=3,
    ...
    def load_data(self):
        ...
        if 'train' in self.split:
            folder_name_list = ['fern','flower','room','trex','horns','fortress','leaves','orchids']
            for folder_name in folder_name_list:
                gt_urls1 = '{}/{}/images_4'.format(self.dataPath,folder_name)
                
                per_seq = []
                # print(os.path.join(gt_urls1,'*.png'))
                gt_files = sorted(glob.glob(os.path.join(gt_urls1,'*.png')))
                for idx,gt_url in enumerate(gt_files) : 
                    if idx % 8 ==0:
                        continue 
                    per_seq.append(gt_url)
                self.file_folder_list.append(per_seq)

                gt_urls2 = '{}/{}/images_8'.format(self.dataPath,folder_name)
                per_seq = []
                gt_files = sorted(glob.glob(os.path.join(gt_urls2,'*.png')))
                for idx,gt_url in enumerate(gt_files) : 
                    if idx % 8 ==0:
                        continue 
                    per_seq.append(gt_url)
                self.file_folder_list.append(per_seq)
        # print(len(self.file_folder_list))
    ...
    def __getitem__(self,index):
        ...
        img_files = self.file_folder_list[index]
        # print('img_files',img_files)

        idx_list_full = [i for i in range(len(img_files))]
        ...
        if 'train' in self.split:
            ...
            idx_list = [1,2,3]
        frame_hr = [cv2.imread(img_files[idx]) for idx in idx_list] 
